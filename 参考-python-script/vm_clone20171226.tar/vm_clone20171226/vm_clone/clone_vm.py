...
def creat_cf_list():
    ...
    for key in cf.keys():
        ...
        print stage_2,num,clone_cluster_2
        ...
        for num_AA in range(0,num):
            ...
            if stat_app != None:
                app = stat_app
                clone_str = "python get_clone.py -t %s -b %s -d '%s' -H %s -c %s -i %s -m %s -a %s"%(template_name,base_name_new,datastore_name,the_host,the_stage,ip_addr,vm_hostname_new,app)
            else:
                clone_str = "python get_clone.py -t %s -b %s -d '%s' -H %s -c %s -i %s -m %s"%(template_name,base_name_new,datastore_name,the_host,the_stage,ip_addr,vm_hostname_new)
            #print clone_str
