...
$IPTABLES -A INPUT -s 10.11.0.0/16 -p tcp -m tcp --dport 22 -j ACCEPT
#office
$IPTABLES -A INPUT -s 10.11.0.0/16 -p tcp -m state --state NEW -m tcp -j ACCEPT
$IPTABLES -A INPUT -s 10.13.0.0/16 -p tcp -m state --state NEW -m tcp -j ACCEPT
$IPTABLES -A INPUT -s 49.65.2.128/16 -p tcp -m state --state NEW -m tcp -j ACCEPT
$IPTABLES -A INPUT -s 153.3.251.202/16 -p tcp -m state --state NEW -m tcp -j ACCEPT
# FTP
...
restrict ntp.99bill.com
restrict 127.0.0.1
server ntp.99bill.com minpoll 4 maxpoll 4 prefer
server  127.127.1.0 minpoll 4 maxpoll 4    # local clock
